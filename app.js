...
app.post('/participants', async (req, res) => {
  ...
  try {
    await mongoClient.connect();
    const dbBatePapoUOL = mongoClient.db('batePapoUOL_API');
    const validateNameAvailability = await dbBatePapoUOL.collection('participants').find(req.body).toArray();
    if (validateNameAvailability.length > 0) {
      res.status(409).send('Esse nome já está sendo utilizado');
      mongoClient.close();
    } else {
      await dbBatePapoUOL.collection('participants').insertOne({ name, lastStatus: Date.now() });
      await dbBatePapoUOL.collection('messages').insertOne({
        from: name,
        to: 'Todos',
        text: 'entra na sala...',
...
app.get('/messages', async (req, res) => {
  ...
  const dbBatePapoUOL = mongoClient.db('batePapoUOL_API');
  try {
    const messages = await dbBatePapoUOL.collection('messages').find({
      $or: [{ from: user }, { to: user }, { to: 'Todos' }, { type: 'message' }],
    }).toArray();
    if (messages < limit || Number.isNaN(limit) || limit < 0) {
